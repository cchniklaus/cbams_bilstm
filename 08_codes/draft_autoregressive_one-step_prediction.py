...
# compute statistics
statistics_df = cbms.compute_statistics(all_data_df)
statistics_df.to_csv(base_dir + "/06_results/statistics.csv",index=False)
...
transform_data_list = [cbms.transform_data(i,dim_x=60,dim_y=1) for i in norm_data_list]
# save
for i in range(len(transform_data_list)):
    ...
    x_y_train_df.to_csv(base_dir + "/03_train_valida_test/%s_x_y_train.csv"% name,index=False)
    x_y_valida_df.to_csv(base_dir + "/03_train_valida_test/%s_x_y_valida.csv"% name,index=False)
    x_y_test_df.to_csv(base_dir + "/03_train_valida_test/%s_x_y_test.csv"% name,index=False)
...
for i in range(len(data_name)):
    name = data_name[i]
    x_y_train_df = pd.read_csv(base_dir + "/03_train_valida_test/%s_x_y_train.csv"% name)
    x_y_valida_df = pd.read_csv(base_dir + "/03_train_valida_test/%s_x_y_valida.csv"% name)

    x_y_train = x_y_train_df.values
    x_y_valida = x_y_valida_df.values

    len_t = x_y_train.shape[1]-1
    num_feats = 1

    input_layer = Input(batch_shape=(None,len_t,num_feats))
    model_list = [cbms.bilstm,cbms.bilstm_cbam]
    output_layer_list = [model_list[i](input_layer) for i in range(len(model_list))]
    for i in range(1,16,1):
        output_layer_list.append(cbms.cbams_bilstm(input_layer,cbam_n=i))
    model_name = ["bilstm","bilstm_cbam"] + ["cbams_bilstm_%d"% i for i in range(1,16,1)]

    for i in range(len(output_layer_list)):
        cbms.train_model(input_layer=input_layer,
                         output_layer=output_layer_list[i],
                         x_y_train=(x_y_train[:,:-1],x_y_train[:,-1]),
                         x_y_valida=(x_y_valida[:,:-1],x_y_valida[:,-1]),
                         loss_path=base_dir + "/04_loss/%s_%s_mse.png"% (name,model_name[i]),
                         model_path=base_dir + "/05_model/%s_%s.h5"% (name,model_name[i]))
...
for i in range(len(data_name)):
    print(i)
    name = data_name[i]
    x_y_test_df = pd.read_csv(base_dir + "/03_train_valida_test/%s_x_y_test.csv"% name)
    x_y_test = x_y_test_df.values

    len_t = x_y_test.shape[1]-1
    num_feats = 1
    ...
    for j in range(len(model_name)):
        model = load_model(base_dir + "/05_model/%s_%s.h5"% (name,model_name[j]))
        # predict
        pred_y = model.predict(x_y_test[:,:-1].reshape((-1,len_t,num_feats)))
        if model_name[j] != "bilstm_cbam":
            pred_y_dict[model_name[j]] = np.squeeze(pred_y)
        # evaluate
        metrics_results,metrics_name = cbms.evaluate_model(x_y_test[:,-1],pred_y.flatten())
    ...
    results_df.to_csv(base_dir + "/06_results/%s_metrics.csv"% name,index=False)
    # compute mcs test
    error_list,error_name = mcs.error(real_y=x_y_test[:,-1],pred_y=pred_y_dict)
...
mcs_df = pd.DataFrame(mcs_dict,index=["bilstm"] + ["cbams_bilstm_%d"% i for i in range(1,16,1)])
mcs_df.to_csv(base_dir + "/06_results/mcs.csv",index=True)
...
for i in range(len(data_name)):
    ...
    output_layer_list = [model_list[i](input_layer) for i in range(len(model_list))]
    model_name = ["cnn","lstm","bilstm","cnn_lstm","cnn_bilstm"]
...
for i in range(len(data_name)):
    ...
    num_feats = 1
    # load model
    model_name = ["cnn","lstm","bilstm","cnn_lstm","cnn_bilstm","cbams_bilstm_15"]
    model_list = []
    pred_y_dict = {}
    metrics_results_list = []
    ...
    #save metrics
    results_df = pd.DataFrame(metrics_results_list,columns=["model"]+metrics_name)
    results_df.to_csv(base_dir + "/06_results/%s_metrics_Autoregressive_one-step_prediction.csv"% name,index=False)
    # compute mcs test
    error_list,error_name = mcs.error(real_y=x_y_test[:,-1],pred_y=pred_y_dict)
    mcs_results.append([i.reshape((-1,i.shape[0],i.shape[1])) for i in error_list])
...
for stats in ["R","MAX"]:
    for j in range(len(error_name)):
        ...
        name = error_name[j] + "_" + stats
        mcs_dict[name] = np.squeeze(mcs.mcs_test(error,stats))
mcs_df = pd.DataFrame(mcs_dict,index=["cnn","lstm","bilstm","cnn_lstm","cnn_bilstm","cbams_bilstm_15"])
mcs_df.to_csv(base_dir + "/06_results/mcs_Autoregressive_one-step_prediction.csv",index=True)
